...
<body class="bg-[#f9fff9] text-[#2d3e2d] font-sans">
    ...
    <div class='px-32'>
        <h1 class="text-4xl font-bold text-center my-8 text-[#267326]">Technician Assignment Management</h1>

        <div class="flex flex-col md:flex-row gap-8 justify-center">
            <!-- Technician Assignment Form -->
            <div class="flex-1 bg-white p-6 rounded-lg shadow-lg border border-[#b2d8b2]">
                <h2 class="text-2xl font-semibold mb-4 text-[#2d582d]">Assign Technician to a Ticket</h2>
                <form method="POST" action="" class="space-y-6">
                    <!-- Select Technician -->
                    <div>
                        <label for="technician_id" class="block text-sm font-bold text-[#3a603a]">Select Technician:</label>
                        <select id="technician_id" name="technician_id" class="mt-1 block w-full border border-[#d9ecd9] rounded-md shadow-sm bg-[#f3fff3]" required>
                    ...
                    <!-- Select Ticket -->
                    <div>
                        <label for="ticket_id" class="block text-sm font-bold text-[#3a603a]">Select Ticket:</label>
                        <select id="ticket_id" name="ticket_id" class="mt-1 block w-full border border-[#d9ecd9] rounded-md shadow-sm bg-[#f3fff3]" required>
                    ...
                    <button type="submit" name="assign_technician" class="bg-[#267326] text-white px-4 py-2 rounded shadow hover:bg-[#154215]">
                        Assign Technician
                    </button>
                </form>
            </div>
            ...
            <div class="flex-1 bg-white p-6 rounded-lg shadow-lg border border-[#b2d8b2]">
                <h2 class="text-2xl font-semibold mb-4 text-[#2d582d]">Disassign Technician from a Ticket</h2>
                <form method="POST" action="" class="space-y-6">
                    <!-- Select Technician -->
                    <div>
                        <label for="technician_id" class="block text-sm font-bold text-[#3a603a]">Select Technician:</label>
                    ...
                    <div>
                        ...
                        <select id="ticket_id" name="ticket_id" class="mt-1 block w-full border border-[#d9ecd9] rounded-md shadow-sm bg-[#f3fff3]" required>
                            <option value="">Select a Ticket</option>
                            <?php
                            $ticket_query = $conn->query("SELECT Ticket_ID, Description_Of_Issue FROM Ticket WHERE Technician_ID IS NOT NULL");
                            while ($row = $ticket_query->fetch_assoc()) {
                                echo "<option value='{$row['Ticket_ID']}'>
                                        Ticket ID: {$row['Ticket_ID']} - {$row['Description_Of_Issue']}
                                      </option>";
                    ...
                    <button type="submit" name="disassign_technician" class="bg-[#267326] text-white px-4 py-2 rounded shadow hover:bg-[#154215]">
        ...
        <!-- Ticket Table -->
        <div class="mt-10 bg-white p-6 rounded-lg shadow-lg border border-[#b2d8b2]">
            <h2 class="text-2xl font-semibold mb-4 text-[#2d582d]">Tickets Table</h2>
            <?php
            $query = "
                SELECT 
                    Ticket.Ticket_ID, 
            ...
            if ($result->num_rows > 0) {
                ...
                echo "
                    <thead>
                        <tr class='bg-[#267326] text-white'>
                            <th class='border border-[#b2d8b2] px-4 py-2'>Ticket ID</th>
                            <th class='border border-[#b2d8b2] px-4 py-2'>Customer Name</th>
                            <th class='border border-[#b2d8b2] px-4 py-2'>Technician Name</th>
                            <th class='border border-[#b2d8b2] px-4 py-2'>Device Serial Number</th>
                            <th class='border border-[#b2d8b2] px-4 py-2'>Description of Issue</th>
                            <th class='border border-[#b2d8b2] px-4 py-2'>Date of Assignment</th>
                            <th class='border border-[#b2d8b2] px-4 py-2'>Date of Completion</th>
                        </tr>
                    </thead>
                    <tbody>";
                while ($row = $result->fetch_assoc()) {
                    ...
                    echo "<tr class='hover:bg-[#f3fff3]'>
                            <td class='border border-[#b2d8b2] px-4 py-2'>{$row['Ticket_ID']}</td>
                            <td class='border border-[#b2d8b2] px-4 py-2'>{$row['Customer_First_Name']} {$row['Customer_Last_Name']}</td>
                            <td class='border border-[#b2d8b2] px-4 py-2'>{$technician_name}</td>
                            <td class='border border-[#b2d8b2] px-4 py-2'>{$row['Device_Serial_Number']}</td>
                            <td class='border border-[#b2d8b2] px-4 py-2'>{$row['Description_Of_Issue']}</td>
                            <td class='border border-[#b2d8b2] px-4 py-2'>{$row['Date_Of_Assignment']}</td>
                            <td class='border border-[#b2d8b2] px-4 py-2'>{$row['Date_Of_Completion']}</td>
                          </tr>";
                }
                echo "</tbody></table>";
            } else {
                echo "<p class='text-[#3a603a]'>No tickets available.</p>";
            }
            ?>
        </div>
    </div>
